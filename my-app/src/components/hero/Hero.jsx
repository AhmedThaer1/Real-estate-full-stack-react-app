import React from 'react'
import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { AiOutlineSearch } from 'react-icons/ai'
import classes from './hero.module.css'

const Hero = () => {
  const [type, setType] = useState("beach")
  const [continent, setContinent] = useState("0")
  const [priceRange, setPriceRange] = useState("0")
  const navigate = useNavigate()

  // TODO here or somewhere home(fetching properties)

  const handleSearch = () => {
    // navigating to properties
    navigate(`/properties?type=${type}&continent=${continent}&priceRange=${priceRange}`)
  }

  return (
    <div className={classes.container}>
  <div className={classes.wrapper}>
    <h2>Let me find your dream place right now</h2>
    <h5>Search the best selection of luxury real estate</h5>
    <div className={classes.options}>
      <div className={classes.selectWrapper}>
        <label htmlFor="type">: المنطقه</label>
        <select id="type" className={classes.select} onChange={(e) => setType(e.target.value)}>
          <option value="" disabled>Select type</option>
          {/* edit here */}
          <option value="beach">Beach</option>
          <option value="mountain">Mountain</option>
          <option value="village">Village</option>
          <option value="baghdad">baghdad</option>
        </select>
      </div>
      <div className={classes.selectWrapper}>
        <label htmlFor="priceRange">: السعر </label>
        <select id="priceRange" className={classes.select} onChange={(e) => setPriceRange(e.target.value)}>
          <option value="" disabled>Select Price Range</option>
          {/* edit here */}
          <option value="0">0-100,000</option>
          <option value="1">100,000-200,000</option>
          <option value="2">200,000-300,000</option>
          <option value="3">300,000-400,000</option>
          <option value="4">400,000-500,000</option>
        </select>
      </div>
      <div className={classes.selectWrapper}>
        <label htmlFor="continent">: المدينه</label>
        <select id="continent" className={classes.select} onChange={(e) => setContinent(e.target.value)}>
          <option value="" disabled>Select Continent</option>
          {/* edit here */}
          <option value="0">Europe</option>
          <option value="1">Asia</option>
          <option value="2">Africa</option>
          <option value="3">South America</option>
          <option value="4">North America</option>
          <option value="5">Oceania</option>
          <option value="6">Baghdad</option>
        </select>
      </div>
      <button className={classes.button}>
        <span className={classes.spn}>
          <AiOutlineSearch className={classes.searchIcon} onClick={handleSearch} />
        </span>
      </button>
    </div>
  </div>
</div>

  )
}

export default Hero